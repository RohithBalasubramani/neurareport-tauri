...
export class AgentBrain {
  ...
  // Current scenario ID
  private scenarioId = ''
  private qaProfile: QaAgentProfile = 'neurareport'
...
RESPOND WITH A JSON OBJECT:
{
  ...
  "confidence": 0.0,
  "type": "click|type|navigate|scroll|select|wait|key|screenshot|done",
...
${this.config.useVision ? '- Screenshot paths are provided — view them to see the actual screen.' : ''}`
  ...
  private normalizeAction(action: AgentAction, observation: PageObservation): AgentAction {
    ...
    if (['click', 'type', 'select', 'upload'].includes(normalized.type) && !normalized.target) {
